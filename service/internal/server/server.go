...
func New(opts Options) *fiber.App {
	...
	if coreAPIBase != "" {
		if v, err := coreauth.NewVerifier(coreauth.Config{
			...
			fmt.Printf("warn: sheets coreauth verifier init failed: %v\n", err)
		} else {
			authVerifier = v
		}
	}
		...
		if token != "" && authVerifier != nil {
			if claims, err := authVerifier.Verify(token); err == nil {
				...
				if !errors.Is(err, coreauth.ErrJWKSUnavailable) {
					fmt.Printf("warn: sheets coreauth verify failed: %v\n", err)
				}
			}
		}
	...
	setStatus := func(c *fiber.Ctx, status string) error {
		...
		id := c.Params("id")
		var s Sheet
		if err := opts.DB.Get(&s, `UPDATE sheets SET status=$1, updated_at=now() WHERE id=$2 AND (
        user_id=$3 OR EXISTS(SELECT 1 FROM sheet_collaborators sc WHERE sc.sheet_id=$2 AND sc.user_id=$3 AND sc.role='editor')
      )
	...
	// List
	app.Get("/v1/sheets", func(c *fiber.Ctx) error {
		if opts.DB == nil {
			return c.Status(500).JSON(fiber.Map{"success": false})
		}
		uid, err := getUID(c)
		...
		q := `SELECT id, user_id, title, COALESCE(data,'null'::jsonb) AS data, status, created_at, updated_at FROM sheets s
              WHERE (s.user_id=$1 OR EXISTS (SELECT 1 FROM sheet_collaborators c WHERE c.sheet_id=s.id AND c.user_id=$1))
	...
	// Get
	app.Get("/v1/sheets/:id", func(c *fiber.Ctx) error {
		if opts.DB == nil {
			return c.Status(500).JSON(fiber.Map{"success": false})
		}
		uid, err := getUID(c)
		...
		id := c.Params("id")
		var s Sheet
		if err := opts.DB.Get(&s, `SELECT id, user_id, title, COALESCE(data,'null'::jsonb) AS data, status, created_at, updated_at FROM sheets s
            WHERE s.id=$1 AND (s.user_id=$2 OR EXISTS (SELECT 1 FROM sheet_collaborators c WHERE c.sheet_id=s.id AND c.user_id=$2))`, id, uid); err != nil {
			return c.Status(404).JSON(fiber.Map{"success": false, "message": err.Error()})
		}
		return c.JSON(fiber.Map{"success": true, "data": s})
	})
	...
	// Create
	app.Post("/v1/sheets", func(c *fiber.Ctx) error {
		if opts.DB == nil {
			return c.Status(500).JSON(fiber.Map{"success": false})
		}
		uid, err := getUID(c)
		...
		var s Sheet
		if err := opts.DB.Get(&s, `INSERT INTO sheets (user_id, title, data, status)
	...
	// Update
	app.Put("/v1/sheets/:id", func(c *fiber.Ctx) error {
		if opts.DB == nil {
			return c.Status(500).JSON(fiber.Map{"success": false})
		}
		uid, err := getUID(c)
		...
		var s Sheet
		if err := opts.DB.Get(&s, `UPDATE sheets SET
            title = NULLIF($1,''), data = NULLIF($2,'null'::jsonb), status = COALESCE(NULLIF($3,''), status), updated_at = now()
            WHERE id=$4 AND (
              user_id=$5 OR EXISTS (SELECT 1 FROM sheet_collaborators sc WHERE sc.sheet_id=$4 AND sc.user_id=$5 AND sc.role='editor')
            )
	...
	app.Post("/v1/sheets/:id/archive", func(c *fiber.Ctx) error { return setStatus(c, "archived") })
	app.Post("/v1/sheets/:id/restore", func(c *fiber.Ctx) error { return setStatus(c, "active") })
	app.Delete("/v1/sheets/:id", func(c *fiber.Ctx) error { return setStatus(c, "deleted") })

	// Collaborators (owner-managed)
	app.Get("/v1/sheets/:id/collaborators", func(c *fiber.Ctx) error {
		if opts.DB == nil {
			return c.Status(500).JSON(fiber.Map{"success": false})
		}
		uid, err := getUID(c)
		...
		id := c.Params("id")
		var owner string
		if err := opts.DB.Get(&owner, `SELECT user_id FROM sheets WHERE id=$1`, id); err != nil {
		...
		rows := []row{}
		_ = opts.DB.Select(&rows, `SELECT user_id, role, invited_by, created_at FROM sheet_collaborators WHERE sheet_id=$1 ORDER BY created_at DESC`, id)
		return c.JSON(fiber.Map{"success": true, "data": rows})
	})
	app.Post("/v1/sheets/:id/collaborators", func(c *fiber.Ctx) error {
		if opts.DB == nil {
			return c.Status(500).JSON(fiber.Map{"success": false})
		}
		uid, err := getUID(c)
		...
		if _, err := opts.DB.Exec(`INSERT INTO sheet_collaborators (sheet_id, user_id, role, invited_by) VALUES ($1,$2,$3,$4)
		  ON CONFLICT (sheet_id, user_id) DO UPDATE SET role=EXCLUDED.role, updated_at=now()`, id, body.UserID, role, uid); err != nil {
			return c.Status(500).JSON(fiber.Map{"success": false, "message": err.Error()})
		}
		return c.JSON(fiber.Map{"success": true})
	})
	app.Delete("/v1/sheets/:id/collaborators", func(c *fiber.Ctx) error {
		if opts.DB == nil {
			return c.Status(500).JSON(fiber.Map{"success": false})
		}
		uid, err := getUID(c)
		...
		if _, err := opts.DB.Exec(`DELETE FROM sheet_collaborators WHERE sheet_id=$1 AND user_id=$2`, id, userID); err != nil {
			return c.Status(500).JSON(fiber.Map{"success": false, "message": err.Error()})
		}
		return c.JSON(fiber.Map{"success": true})
	})
