...
func New(opts Options) *fiber.App {
    ...
    app.Get("/v1/sheets", func(c *fiber.Ctx) error {
        ...
        q := `SELECT id, user_id, title, COALESCE(data,'null'::jsonb) AS data, status, created_at, updated_at FROM sheets
    ...
    app.Get("/v1/sheets/:id", func(c *fiber.Ctx) error {
        if opts.DB == nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        uid, err := getUID(c); if err != nil { return c.Status(401).JSON(fiber.Map{"success": false}) }
        id := c.Params("id"); var s Sheet
        if err := opts.DB.Get(&s, `SELECT id, user_id, title, COALESCE(data,'null'::jsonb) AS data, status, created_at, updated_at FROM sheets WHERE id=$1 AND user_id=$2`, id, uid); err != nil {
            return c.Status(404).JSON(fiber.Map{"success": false, "message": err.Error()})
        }
        return c.JSON(fiber.Map{"success": true, "data": s})
    })

    type sheetIn struct { Title *string `json:"title"`; Data json.RawMessage `json:"data"`; Status *string `json:"status"` }
    // Create
    app.Post("/v1/sheets", func(c *fiber.Ctx) error {
        if opts.DB == nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        uid, err := getUID(c); if err != nil { return c.Status(401).JSON(fiber.Map{"success": false}) }
        var in sheetIn; if err := c.BodyParser(&in); err != nil { return c.Status(400).JSON(fiber.Map{"success": false}) }
    ...
    app.Put("/v1/sheets/:id", func(c *fiber.Ctx) error {
        if opts.DB == nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        uid, err := getUID(c); if err != nil { return c.Status(401).JSON(fiber.Map{"success": false}) }
        id := c.Params("id"); var in sheetIn; if err := c.BodyParser(&in); err != nil { return c.Status(400).JSON(fiber.Map{"success": false}) }
        var s Sheet
...
func setStatus(opts Options, c *fiber.Ctx, status string) error {
    ...
    id := c.Params("id")
    var s Sheet
    if err := opts.DB.Get(&s, `UPDATE sheets SET status=$1, updated_at=now() WHERE id=$2 AND user_id=$3
