...
export class SheetsService {
  private cache: Record<string, SheetDoc> = {};
  ...
  async save(s: SheetDoc): Promise<SheetDoc|undefined> {
  ...
  async archive(id: string) { if (this.preferRemote) { try { this.beginSave(); await firstValueFrom(this.http.post(`${API_BASE}/v1/sheets/${id}/archive`, {}, { withCredentials: true })); this.endSave(); } catch (e) { this.endSave(e); this.switchToLocal(e); } } const s=this.cache[id]; if (s){ s.status='archived'; s.updatedAt=this.now(); this.persist(); } }
  async restore(id: string) { if (this.preferRemote) { try { this.beginSave(); await firstValueFrom(this.http.post(`${API_BASE}/v1/sheets/${id}/restore`, {}, { withCredentials: true })); this.endSave(); } catch (e) { this.endSave(e); this.switchToLocal(e); } } const s=this.cache[id]; if (s){ s.status='active'; s.updatedAt=this.now(); this.persist(); } }
...
export function defaultGrid(rows=20, cols=10): string[][] {
